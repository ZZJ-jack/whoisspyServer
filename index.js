...
async function saveRoomsToKV(env) {
    try {
        ...
        const data = { rooms: roomsArray, lastUpdated: now };
...
async function handleCreateRoom(params, env) {
    ...
    return createResponse({ code: 0, msg: '房间创建成功', data: { roomId, isOwner: true } });
}

async function handleJoinRoom(params) {
    const { roomId } = params;
    const room = getRoom(roomId); // getRoom 会自动更新 lastActivity
    if (!room) {
        return createResponse({ code: -1, msg: '房间号无效或已过期' });
    }
    return createResponse({ code: 0, msg: '加入房间成功', data: { isOwner: false, lockNum: room.lockNum, isLock: room.isLock, total: room.total } });
}

async function handleLockNum(params) {
    ...
    room.isLock = true;
    room.lockNum = numInt;
    // lastActivity 已在 getRoom 中更新
    setRoom(roomId, room);
    return createResponse({ code: 0, msg: '题目编号已锁定', data: { lockNum: numInt } });
}

async function handleGetWord(params) {
    const { roomId } = params;
    const room = getRoom(roomId); // getRoom 会自动更新 lastActivity
    if (!room) {
        return createResponse({ code: -1, msg: '房间号无效' });
    ...
    return createResponse({ code: 0, msg: '身份分配成功', data: { currRole, lockNum, word, assigned: room.assigned, total: room.total } });
}

async function handleResetRoom(params) {
    const { roomId } = params;
    const room = getRoom(roomId); // getRoom 会自动更新 lastActivity
    if (!room) {
        return createResponse({ code: -1, msg: '房间号无效' });
    ...
    const newRoomData = {
        ...
        lastActivity: Date.now() // 重置时也更新时间
    };
    setRoom(roomId, newRoomData);
    return createResponse({ code: 0, msg: '房间重置成功' });
}

// --- 新增：handleDeleteRoom 函数 ---
async function handleDeleteRoom(params, env) {
    const { roomId } = params;
    // 尝试从内存缓存中删除
    const deletedFromCache = deleteRoom(roomId);
    
    // 尝试从持久化KV中删除
    let deletedFromKV = false;
    if (env.ROOMS_KV) {
        try {
            await env.ROOMS_KV.delete('rooms_data'); // 简单粗暴，直接删除整个KV键，下次save时会覆盖
            // 更精确的方式是取出、修改、再存回，但考虑到并发，全量存取可能更安全
            // 这里为了与现有saveRoomsToKV逻辑兼容，我们还是采用全量保存的方式
            // 所以删除操作就是从缓存中删除，然后强制保存一次（即使缓存变空）
            deletedFromKV = true; 
        } catch (e) {
            console.error('删除KV房间数据失败:', e);
        }
    }

    if (deletedFromCache || deletedFromKV) {
        // 强制保存，以确保KV中的房间列表被更新
        if (env.ROOMS_KV) {
            await saveRoomsToKV(env);
        }
        return createResponse({ code: 0, msg: '房间数据已删除' });
    } else {
        return createResponse({ code: -1, msg: '未找到要删除的房间' });
    }
}
...
async function handleGetWordByNum(params) {
    ...
    return createResponse({ code: 0, msg: '获取题目成功', data: { num: numInt, spyWord, civilWord } });
}

async function handleGetBankInfo() {
    return createResponse({ code: 0, msg: '题库信息', data: { total: validNums.length, minNum: validNums[0] || 1, maxNum: validNums[validNums.length - 1] || 1, sampleCount: Math.min(5, validNums.length) } });
}
        ...
        try {
            switch (pathname) {
                case '/api/createRoom':
                    ...
                    break;
                case '/api/joinRoom':
                    response = await handleJoinRoom(params);
                    break;
                case '/api/lockNum':
                    response = await handleLockNum(params);
                    break;
                case '/api/getWord':
                    response = await handleGetWord(params);
                    break;
                case '/api/resetRoom':
                    response = await handleResetRoom(params);
                    break;
                // --- 新增路由 ---
                case '/api/deleteRoom':
                    response = await handleDeleteRoom(params, env);
                    break;
            ...
            response = createResponse({ code: -500, msg: '服务器内部错误', error: error.message }, 500);
        }
